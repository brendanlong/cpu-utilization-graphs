...
def analyze_test_type(df, test_type):
    ...
    varying_workers_df = varying_workers_df.with_columns(
        (pl.col("bogo_ops_per_sec") / max_bogo_ops * 100).alias("adjusted_cpu_utilization")
    )
    ...
    print(f"{'Target CPU%':<12} {'Actual CPU%':<12} {'Adjusted CPU%':<15} {'Bogo ops/sec':<15}")
    print("-"*80)
    ...
    for row in sample_rows:
        if row == '...':
            ...
            print(f"{row['cpu_target']:<12.0f} {row['actual_cpu_utilization']:<12.2f} "
    ...
    ax2.plot(cpu_target, mapping_df["actual_cpu_utilization"].to_numpy(), 
             'b-o', alpha=0.7, label='Actual CPU%', markersize=3)
    ax2.plot(cpu_target, mapping_df["adjusted_cpu_utilization"].to_numpy(), 
             'r-s', alpha=0.7, label='Adjusted CPU%', markersize=3)
    ...
    if len(varying_workers_df) > 0:
        ...
        ax4_twin = ax4.twinx()
        line1 = ax4.plot(workers, workers_bogo, 'b-o', alpha=0.7, label='Bogo ops/sec', markersize=4)
        line2 = ax4_twin.plot(workers, workers_adjusted, 'r-s', alpha=0.7, label='Adjusted CPU%', markersize=4)
        ...
        ax4.text(0.5, 0.5, 'No data for varying workers at 100% CPU', 
                ha='center', va='center', transform=ax4.transAxes)
        ax4.set_title(f'Performance vs Workers - {test_type.upper()} (No Data)')
    
    plt.suptitle(f'CPU Utilization Analysis - {test_type.upper()} Test Method', fontsize=14, y=1.02)
    plt.tight_layout()
    plt.savefig(f'cpu_utilization_analysis_{test_type}.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    if len(actual_cpu) > 0:
        mapping_export = pl.DataFrame({
            ...
            "adjusted_cpu_percent": mapping_df["adjusted_cpu_utilization"].to_numpy(),
        ...
        scaling_factors = mapping_df["adjusted_cpu_utilization"].to_numpy() / mapping_df["actual_cpu_utilization"].to_numpy()
        ...
        linearity_error = np.abs(mapping_df["actual_cpu_utilization"].to_numpy() - 
