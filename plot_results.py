...
def plot_bogo_ops_vs_cpu(df: pd.DataFrame, output_prefix: str):
    ...
    for test_type in test_types:
        ...
        if len(test_data) > 1:
            z = np.polyfit(
                test_data["actual_cpu_utilization"], test_data["bogo_ops_per_sec"], 1
            )
            p = np.poly1d(z)
            x_trend = np.linspace(
                test_data["actual_cpu_utilization"].min(),
                test_data["actual_cpu_utilization"].max(),
                100,
            )
    ...
    for test_type in test_types:
        ...
        test_data["efficiency"] = (
            test_data["bogo_ops_per_sec"] / test_data["actual_cpu_utilization"]
        )
...
def generate_summary_report(df: pd.DataFrame, output_file: str):
    ...
    with open(output_file, "w") as f:
        ...
        accuracy = (
            df.groupby("test_type")["cpu_error"].agg(["mean", "std", "max"]).round(2)
        )
